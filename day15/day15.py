...
walls = set()
initial_units = {}
with open('day15//day15.in') as puzzle_file:
...
def nearest(start, goals, excludes):
    ...
    while queue:
        ...
        queue = set(adj for pos in queue for adj in adjs(pos))
...
def dbg(units):
    for y in range(min(y for y, _ in walls), max(y for y, _ in walls) + 1):
        row = ''.join(
            '#' if (y, x) in walls else ('E' if units[(y, x)].is_elf else 'G')
            if (y, x) in units else '.'
            for x in range(min(x for _, x in walls), max(x for _, x in walls) + 1))
        etc = ', '.join('{}({})'.format('E' if unit.is_elf else 'G', unit.hp)
...
for elf_power in range(3, 201):
    ...
    while True:
        ...
        #dbg(units)
        #print('')
        if len(set(unit.is_elf for unit in units.values())) < 2:
            break

        queue = OrderedDict(sorted(units.items(), key=lambda item: item[0]))
        units = {}
        while queue:
            ...
            if not enemies:
                ...
                units[pos] = unit
                break
            if not any(adj in enemies for adj in adjs(pos)):
                enemy_ranges = set(adj for enemy_pos in enemies.keys() for adj in adjs(enemy_pos))
                nearest_enemy_range = nearest(pos, enemy_ranges, excludes)
                if nearest_enemy_range:
                    step = nearest(nearest_enemy_range, adjs(pos), excludes)
                    if step:
                        pos = step
